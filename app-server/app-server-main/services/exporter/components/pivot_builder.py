...
class PivotBuilder:
    ...
    def build_pivots(self):
        ...
        df = self.base.df
        # Filter IS type only once
        is_df = df.loc[df["type"] == "IS"]

        valid_fys = memory_store[df["dataset_id"].iloc[0]]["valid_fiscal_years"]

        # Annual pivot building
        fy_rows = is_df.loc[is_df["fiscal_year"].isin(valid_fys)].copy()
        fy_rows["column_label"] = fy_rows["fiscal_year"]

        ltm_rows = is_df.loc[is_df["ltm"].str.strip().astype(bool)].copy()
        ltm_rows["column_label"] = ltm_rows["ltm"]

        df_combined = pd.concat([fy_rows, ltm_rows], ignore_index=True)
        # Clean only once
        mask_valid_labels = df_combined["column_label"].notna() & df_combined["column_label"].str.strip().astype(bool)
        df_combined = df_combined.loc[mask_valid_labels]
        ...
        # Faster sales debug filter
        sales_idx = [idx for idx in self.base.annual_pivot.index if idx[0].strip().lower() == "sales"]
        if sales_idx:
            print("[DEBUG] Sales rows in pivot (any column):")
            for idx in sales_idx:
                print(f"{idx}: {self.base.annual_pivot.loc[idx].to_dict()}")

        # Sort columns for annual pivot
        cols = list(self.base.annual_pivot.columns)
        ...
        # Monthly pivot
        print("[INFO] Building monthly pivot...")
        monthly_df = is_df.loc[is_df["month"].notna()].copy()
        monthly_df["column_label"] = pd.to_datetime(monthly_df["date"]) + pd.offsets.MonthEnd(0)
        monthly_df["column_label"] = monthly_df["column_label"].dt.strftime("%d-%b-%y")
        ...
        # Quarterly pivot
        print("[INFO] Building quarterly pivot...")
        quarterly_df = is_df.loc[is_df["fiscal_quarter"].notna()].copy()
        quarterly_df["column_label"] = quarterly_df["fiscal_quarter"]
        ...
        # Apply final sort for annual pivot
        self.base.annual_pivot = self.base.annual_pivot.reindex(sorted(cols, key=sort_key), axis=1)
