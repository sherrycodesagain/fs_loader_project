...
class DeltaCalculator:
    ...
    def write_all_delta_formulas(self, ws: Worksheet, start_data_row: int):
        ...
        # Cache styles and alignment
        currency_format = self.base.currency_format
        percent_format = self.base.percent_format
        ...
        for row in range(start_data_row, ws.max_row + 1):
            ...
            # Δ value
            ws.cell(row=row, column=delta_col, value=f"={curr_letter}{row}-{prev_letter}{row}") \
                .number_format = currency_format
            ws.cell(row=row, column=delta_col).alignment = center_align

            # Δ %
            ws.cell(row=row, column=delta_pct_col,
                    value=f"=IF({prev_letter}{row}=0,0,({curr_letter}{row}/{prev_letter}{row})-1)") \
                .number_format = percent_format
            ws.cell(row=row, column=delta_pct_col).alignment = center_align
